...
void SteppingAction::UserSteppingAction(const G4Step *aStep) {
	...
	if (track->
		...
		< settings->MIN_ENERGY_OUTPUT) // killing particles with energy lower than threshold
	{
		const G4int PDG = aStep->GetTrack()->GetParticleDefinition()->GetPDGEncoding();
			...
			if (track->GetParticleDefinition()->GetParticleName() == "e-") {
				if (track->GetKineticEnergy() > 10.0 * keV) {
					...
					if (asciiFile00.is_open()) {
						asciiFile00 << track->GetKineticEnergy() / keV << "  " << track->GetGlobalTime() / microsecond
							<< G4endl;
						asciiFile00.close();
					}
				}
	...
	if (track->GetTrackID()) {
		...
		if (IDpart_not_recorded_yet_elec(ID) && (ID != 1)) {
			...
			if (track->GetParticleDefinition()->GetParticleName() == "gamma") {
				...
				if (creator_process ==
					"eBrem") // check if it is  produced by bremsstrahlung (positron annihiliation is also possible but not wanted to
					// record)
				{
					if (track->GetKineticEnergy() > 10.0 * keV) {
						...
						if (asciiFile11.is_open()) {
							asciiFile11 << track->GetKineticEnergy() / keV << "  "
								<< track->GetGlobalTime() / microsecond << G4endl;
							asciiFile11.close();
						}
					}
				}
