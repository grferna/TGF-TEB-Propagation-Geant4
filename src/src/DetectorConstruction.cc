...
{
void
gtd7_(INTEGER &IYD, // YEAR AND DAY AS YYDDD (day of year from 1 to 365 (or 366))
      REAL &SEC, // UT(SEC)
...
G4VPhysicalVolume *
TGFDetectorConstruction::Construct() {
    ...
    solidWorld = new G4Sphere("world_S", Settings::earthRadius, (Settings::earthRadius + world_max_altitude),
                              0 * degree,
                              360 * degree, 0 * degree, 180 * degree); // earth radius + 100 km
    // World logical
        ...
        if ((innerRad + outerRad) / 2. < (Settings::earthRadius + ALT_MAX_RECORDED * km)) {
            atmosLayers_LV.push_back(
                    new G4LogicalVolume(atmosLayers_S.back(), Airs[jj], "atmosphere_LV_" + std::to_string(jj), nullptr,
                                        nullptr,
                                        nullptr)); // put vaccum to test if angle sampling is OK
    ...
    for (uint idx_alt = 0; idx_alt < altitudes_.size() - 1; idx_alt++) {
        ...
        if (altitude_in_km > alt_max_atmosphere / km) {
            ...
            gtd7_(input_iyd, input_sec, input_alt, input_g_lat, input_g_long, input_lst, input_f107A, input_f107,
                  input_ap,
                  input_mass, output_D, output_T); // MSIS, fortran function call
