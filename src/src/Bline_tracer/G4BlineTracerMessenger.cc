...
//
//
// $Id: G4BlineTracerMessenger.cc 68021 2013-03-13 13:36:07Z gcosmo $
//
// --------------------------------------------------------------------
//
...
G4BlineTracerMessenger::G4BlineTracerMessenger(G4BlineTracer *aBlineTool) {
	...
	fSetMaxTrackingStepCmd = new G4UIcmdWithADoubleAndUnit(
		"/vis/blineTracer/setMaxStepLength", this);
	fSetMaxTrackingStepCmd->SetGuidance(
	...
	fSetDrawColourCmd = new G4UIcmdWith3Vector("/vis/blineTracer/setColour",
		this);
	...
	fSetDrawBlineCmd = new G4UIcmdWithABool("/vis/blineTracer/stockLines",
		this);
	...
	fSetDrawPointsCmd = new G4UIcmdWithABool("/vis/blineTracer/stockPoints",
		this);
	fSetDrawPointsCmd->SetGuidance(
	...
	fSetPointSizeCmd = new G4UIcmdWithADouble("/vis/blineTracer/setPointSize",
		this);
	...
	fResetCmd = new G4UIcmdWithoutParameter(
		"/vis/blineTracer/resetMaterialToBeDrawn", this);
	fResetCmd->SetGuidance(
...
void G4BlineTracerMessenger::SetNewValue(G4UIcommand *command,
	...
	if (command == fBlineCmd) {
		...
		fTheBlineTool->SetMaxTrackingStep(
			fSetMaxTrackingStepCmd->GetNewDoubleValue(newValues));
	}
	else if (command == fSetDrawBlineCmd) {
		fTheBlineTool->GetEventAction()->SetDrawBline(
			fSetDrawBlineCmd->GetNewBoolValue(newValues));
	}
	else if (command == fSetDrawColourCmd) {
		G4ThreeVector vec = fSetDrawColourCmd->GetNew3VectorValue(newValues);
		fTheBlineTool->GetEventAction()->SetDrawColour(
			G4Colour(vec.x(), vec.y(), vec.z()));
	}
	else if (command == fSetDrawPointsCmd) {
		fTheBlineTool->GetEventAction()->SetDrawPoints(
			fSetDrawPointsCmd->GetNewBoolValue(newValues));
	}
	else if (command == fSetPointSizeCmd) {
		fTheBlineTool->GetEventAction()->SetPointSize(
			fSetPointSizeCmd->GetNewDoubleValue(newValues));
	}
