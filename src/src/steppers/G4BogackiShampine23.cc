...
G4BogackiShampine23::G4BogackiShampine23(G4EquationOfMotion *EqRhs,
		G4int noIntegrationVariables, G4bool primary) :
		G4MagIntegratorStepper(EqRhs, noIntegrationVariables), fLastStepLength(
				0.), fAuxStepper(0)
{
	...
	if (primary)
	{
		fAuxStepper = new G4BogackiShampine23(EqRhs, numberOfVariables,
				!primary);
	}
}
...
void G4BogackiShampine23::Stepper(const G4double yInput[],
		const G4double DyDx[], G4double Step, G4double yOut[], G4double yErr[])
{
	G4int i;

	const G4double b21 = 0.5, b31 = 0., b32 = 3.0 / 4.0, b41 = 2.0 / 9.0, b42 =
			1.0 / 3.0, b43 = 4.0 / 9.0;

	const G4double dc1 = b41 - 7.0 / 24.0, dc2 = b42 - 1.0 / 4.0, dc3 = b43
			- 1.0 / 3.0, dc4 = -0.125;
	...
	for (i = 0; i < numberOfVariables; i++)
	{
		...
		yErr[i] = Step
				* (dc1 * DyDx[i] + dc2 * ak2[i] + dc3 * ak3[i] + dc4 * ak4[i]);
