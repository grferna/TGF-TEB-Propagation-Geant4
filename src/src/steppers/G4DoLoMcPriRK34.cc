...
G4DoLoMcPriRK34::G4DoLoMcPriRK34(G4EquationOfMotion *EqRhs, G4int noIntegrationVariables, G4bool primary) :
...
void
G4DoLoMcPriRK34::Stepper(const G4double yInput[], const G4double DyDx[], G4double Step, G4double yOut[],
                         G4double yErr[])
{
  G4int i;
  ...
  const G4double // G4double - only once
    ...
    b41 = 3043.0 / 3528.0, b42 = -3757.0 / 1176.0, b43 = 1445.0 / 441.0,

    b51 = 17617.0 / 11662.0, b52 = -4023.0 / 686.0, b53 = 9372.0 / 1715.0, b54 = -66.0 / 595.0,

    b61 = 29.0 / 238.0, b62 = 0.0, b63 = 216.0 / 385.0, b64 = 54.0 / 85.0, b65 = -7.0 / 22.0,

    dc1 = 363.0 / 2975.0 - b61, dc2 = 0.0 - b62, dc3 = 981.0 / 1750.0 - b63, dc4 = 2709.0 / 4250.0 - b64,
    dc5 = -3.0 / 10.0 - b65, dc6 = -1.0 / 50.0; // end of declaration
  ...
  for (i = 0; i < numberOfVariables; i++) {
    yOut[i] = yIn[i] + Step * (b61 * DyDx[i] + b62 * ak2[i] + b63 * ak3[i] + b64 * ak4[i] + b65 * ak5[i]);
  }
...
G4double
G4DoLoMcPriRK34::DistChord() const
{
...
void
G4DoLoMcPriRK34::SetupInterpolation() {}

void
G4DoLoMcPriRK34::SetupInterpolate(const G4double /* yInput */[], const G4double /* dydx */[], const G4double /* Step */)
{
  // Do Nothing
}

void
G4DoLoMcPriRK34::Interpolate(G4double tau, G4double yOut[])
{
...
void
G4DoLoMcPriRK34::Interpolate(const G4double yInput[], const G4double dydx[], const G4double Step, G4double yOut[],
                             G4double tau)
{
  ...
  bf1                                       = -(162.0 * tau_3 - 504.0 * tau_2 + 551.0 * tau - 238.0) / 238.0, bf2 = 0.0,
  bf3                                       = 27.0 * tau * (27.0 * tau_2 - 70.0 * tau + 51.0) / 385.0,
  bf4                                       = -27 * tau * (27.0 * tau_2 - 50.0 * tau + 21.0)
                                / 85.0, bf5 = 7.0 * tau * (2232.0 * tau_2 - 4166.0 * tau + 1785.0) / 3278.0,
  bf6                                       = tau * (tau - 1.0) * (387.0 * tau - 238.0) / 149.0;

  for (int i = 0; i < numberOfVariables; i++) {
    yOut[i] = yIn[i] + Step * tau
              * (bf1 * dydx[i] + bf2 * ak2[i] + bf3 * ak3[i] + bf4 * ak4[i] + bf5 * ak5[i] + bf6 * ak6[i]);
  }
}
