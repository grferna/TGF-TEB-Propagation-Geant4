...
G4CashKarpRKF45::G4CashKarpRKF45(G4EquationOfMotion *EqRhs, G4int noIntegrationVariables, G4bool primary) :
  G4MagIntegratorStepper(EqRhs, noIntegrationVariables), fLastStepLength(0.), fAuxStepper(0)
{
  const G4int numberOfVariables = std::max(noIntegrationVariables, (((noIntegrationVariables - 1) / 4 + 1) * 4));
...
void
G4CashKarpRKF45::Stepper(const G4double yInput[], const G4double dydx[], G4double Step, G4double yOut[],
                         G4double yErr[])
{
  // const G4int nvar = 6 ;
  ...
  const G4double b21 = 0.2, b31 = 3.0 / 40.0, b32 = 9.0 / 40.0, b41 = 0.3, b42 = -0.9, b43 = 1.2,
  ...
  const G4double dc1 = c1 - 2825.0 / 27648.0, dc3 = c3 - 18575.0 / 48384.0, dc4 = c4 - 13525.0 / 55296.0,
                 dc6 = c6 - 0.25;
...
void
G4CashKarpRKF45::StepWithEst(const G4double *, const G4double *, G4double, G4double *, G4double&, G4double&,
                             const G4double *, G4double *)
{
  G4Exception("G4CashKarpRKF45::StepWithEst()", "GeomField0001", FatalException, "Method no longer used.");
}

/////////////////////////////////////////////////////////////////

G4double
G4CashKarpRKF45::DistChord() const
{
