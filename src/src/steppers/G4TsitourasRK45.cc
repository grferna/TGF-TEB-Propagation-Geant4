...
G4TsitourasRK45::G4TsitourasRK45(G4EquationOfMotion *EqRhs, G4int noIntegrationVariables, G4bool primary) :
  G4MagIntegratorStepper(EqRhs, noIntegrationVariables), fLastStepLength(0.), fAuxStepper(0)
{
  ...
  const G4int numStateVars = std::max(noIntegrationVariables, numStateMax);
...
void
G4TsitourasRK45::Stepper(const G4double yInput[], const G4double dydx[], G4double Step, G4double yOut[],
                         G4double yErr[])
{
  G4int i;
  ...
  const G4double b21 = 0.161,
                 ...
                 b31 = -0.00848065549235698854, b32 = 0.335480655492356989,
                 ...
                 b61 = 5.8614554429464200, b62 = -12.9209693178471093, b63 = 8.1593678985761586, b64 = -0.071584973281400997,
                 b65 = -0.0282690503940683829,

                 b71 = 0.0964607668180652295, b72 = 0.01, b73 = 0.479889650414499575, b74 = 1.37900857410374189,
                 b75 = -3.2900695154360807, b76 = 2.32471052409977398,
...
} // G4TsitourasRK45::Stepper

void
G4TsitourasRK45::SetupInterpolation() // (const G4double *yInput, const G4double *dydx, const G4double Step)
{
  // Nothing to be done
}

void
G4TsitourasRK45::Interpolate(const G4double *yInput, const G4double *dydx, const G4double Step, G4double *yOut,
                             G4double tau)
{
  ...
  bf1 = -1.0530884977290216 * tau *
        (tau - 1.3299890189751412)
        * (tau_2 - 1.4364028541716351 * tau + 0.7139816917074209), bf2 = 0.1017 * tau_2
                                                                         * (tau_2 - 2.1966568338249754 * tau +
                                                                            1.2949852507374631),
  bf3 = 2.490627285651252793 * tau_2 * (tau_2 - 2.38535645472061657 * tau + 1.57803468208092486),
  bf4 = -16.54810288924490272
        * (tau - 1.21712927295533244) * (tau - 0.61620406037800089) * tau_2,
  bf5 = 47.37952196281928122 * (tau - 1.203071208372362603) * (tau - 0.658047292653547382) * tau_2, bf6 =
    -34.87065786149660974 * (tau - 1.2) * (tau - 0.666666666666666667) * tau_2,
...
G4double
G4TsitourasRK45::DistChord() const
{
