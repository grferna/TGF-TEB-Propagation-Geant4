...
G4DormandPrince745::G4DormandPrince745(G4EquationOfMotion *EqRhs, G4int noIntegrationVariables, G4bool primary) :
  G4MagIntegratorStepper(EqRhs, noIntegrationVariables)
{
  const G4int numberOfVariables = // noIntegrationVariables;
                                  std::max(noIntegrationVariables, (((noIntegrationVariables - 1) / 4 + 1) * 4));
  ...
  const G4int numStateVars = std::max(noIntegrationVariables, std::max(GetNumberOfStateVariables(), 8));
  ...
  if (primary) {
    fAuxStepper = new G4DormandPrince745(EqRhs, numberOfVariables, !primary);
  }
}
...
void
G4DormandPrince745::Stepper(const G4double yInput[], const G4double DyDx[], G4double Step, G4double yOut[],
                            G4double yErr[])
{
  G4int i;
  ...
  // b7j are the coefficients of higher order
    ...
    dc5 = -(b75 + 92097.0 / 339200.0), dc6 = -(b76 - 187.0 / 2100.0), dc7 =
    -(-1.0 / 40.0); // end of declaration
...
G4double
G4DormandPrince745::DistLine(G4double yStart[], G4double yMid[], G4double yEnd[]) const
{
...
G4double
G4DormandPrince745::DistChord2() const
{
...
G4double
G4DormandPrince745::DistChord() const
{
  // Coefficients for halfway interpolation
  const G4double hf1 = 5783653.0 / 57600000.0, hf2 = 0., hf3 = 466123.0 / 1192500.0, hf4 = -41347.0 / 1920000.0,
                 hf5 = 16122321.0 / 339200000.0, hf6 = -7117.0 / 20000.0, hf7 = 183.0 / 10000.0;

  for (int i = 0; i < 3; i++) {
    fMidVector[i] = fLastInitialVector[i] + fLastStepLength
                    * (hf1 * fInitialDyDx[i] + hf2 * ak2[i] + hf3 * ak3[i] + hf4 * ak4[i] + hf5 * ak5[i] + hf6 * ak6[i] + hf7
                       * ak7[i]);
  }
...
G4double
G4DormandPrince745::DistChord3() const
{
  // Do half a step using StepNoErr
  fAuxStepper->Stepper(fLastInitialVector, fInitialDyDx, 0.5 * fLastStepLength, fAuxStepper->fMidVector,
                       fAuxStepper->fMidError);
  return DistLine(fLastInitialVector, fAuxStepper->fMidVector, fLastFinalVector);
}
...
void
G4DormandPrince745::SetupInterpolation_low() // const G4double *yInput, const G4double *dydx, const G4double Step)
{
  // Nothing to be done
}

void
G4DormandPrince745::Interpolate_low( /* const G4double yInput[],
                                      * const G4double dydx[],
                                      * const G4double Step, */
  G4double yOut[], G4double tau)
{
  ...
  const G4double tau_2 = tau * tau, tau_3 = tau * tau_2, tau_4 = tau_2 * tau_2;

  bf1
    = (157015080.0 * tau_4 - 13107642775.0 * tau_3 + 34969693132.0 * tau_2 - 32272833064.0 * tau + 11282082432.0)
      / 11282082432.0,
  bf2                                                                                                  =
    0.0, bf3                                                                                           = -100.0 * tau
                                                                                                         * (
    15701508.0 * tau_3 - 914128567.0 * tau_2 + 2074956840.0 * tau - 1323431896.0) / 32700410799.0, bf4 = 25.0 * tau
                                                                                                         * (
    94209048.0 * tau_3 - 1518414297.0 * tau_2 + 2460397220.0 * tau - 889289856.0) / 5641041216.0,
  bf5
                                                  = -2187.0 * tau
      * (52338360.0 * tau_3 - 451824525.0 * tau_2 + 687873124.0 *
         tau - 259006536.0) / 199316789632.0, bf6 = 11.0 * tau
                                                    * (
    106151040.0 * tau_3 - 661884105.0 * tau_2 + 946554244.0 * tau - 361440756.0) / 2467955532.0,
  ...
  for (int i = 0; i < numberOfVariables; i++) {
    yOut[i] = yIn[i] + Step * tau
              * (bf1 * dydx[i] + bf2 * ak2[i] + bf3 * ak3[i] + bf4 * ak4[i] + bf5 * ak5[i] + bf6 * ak6[i] + bf7 * ak7[i]);
  }
}
...
void
G4DormandPrince745::SetupInterpolation_high( /* const G4double yInput[],
                                              * const G4double dydx[],
                                              * const G4double Step */
  )
{
  ...
  const G4double b81 = 6245.0 / 62208.0, b82 = 0.0, b83 = 8875.0 / 103032.0, b84 = -125.0 / 1728.0,
                 ...
                 b95 = -15975.0 / 108544.0, b96 = 8295925.0 / 220286304.0, b97 = -1779595.0 / 62938944.0, b98 =
    -805.0 / 4104.0;
...
void
G4DormandPrince745::Interpolate_high( /* const G4double yInput[],
                                       * const G4double dydx[],
                                       * const G4double Step, */
  G4double yOut[], G4double tau)
{
  ...
  for (int i = 0; i < numberOfVariables; i++) { // Here i IS the cooridnate no.
    yOut[i] = yIn[i] + stepLen
              * (b[1] * dydx[i] + b[2] * ak2[i] + b[3] * ak3[i] + b[4] * ak4[i] + b[5] * ak5[i] + b[6] * ak6[i] + b[7]
                 * ak7[i] + b[8] * ak8[i] + b[9] * ak9[i]);
  }
} // G4DormandPrince745::Interpolate_high
