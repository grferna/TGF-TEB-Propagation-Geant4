...
#include "C/nrlmsise-00.h" /* header for nrlmsise-00.h */
...
void
tselec(struct nrlmsise_flags *flags) {
    int i;
...
void
glatf(double lat, double *gv, double *reff) {
...
double
ccor(double alt, double r, double h1, double zh) {
...
double
ccor2(double alt, double r, double h1, double zh, double h2) {
    /*        CHEMISTRY/DISSOCIATION CORRECTION FOR MSIS MODELS
     *         ALT - altitude
     *         R - target ratio
     *         H1 - transition scale length
...
double
scalh(double alt, double xm, double temp) {
    double g;
    double rgas = 831.4;
...
double
dnet(double dd, double dm, double zhm, double xmm, double xm) {
...
void
splini(double *xa, double *ya, double *y2a, int n, double x, double *y) {
    ...
    while ((x > xa[klo]) && (khi < n)) {
        ...
        b2 = b * b;
        yi +=
                ((1.0 - a2) * ya[klo] / 2.0 + b2 * ya[khi] / 2.0
                 + ((-(1.0 + a2 * a2) / 4.0 + a2 / 2.0) * y2a[klo] + (b2 * b2 / 4.0 - b2 / 2.0) * y2a[khi]) * h * h /
                   6.0) * h;
        klo++;
        khi++;
    }
...
spline(double *x, double *y, int n, double yp1, double ypn, double *y2) {
    ...
    for (i = 1; i < (n - 1); i++) {
        ...
        y2[i] = (sig - 1.0) / p;
        u[i] =
                (6.0 * ((y[i + 1] - y[i]) / (x[i + 1] - x[i]) - (y[i] - y[i - 1]) / (x[i] - x[i - 1]))
...
__inline_double
zeta(double zz, double zl) {
    return ((zz - zl) * (re + zl) / (re + zz));
}

double
densm(double alt, double d0, double xm, double *tz, int mn3, double *zn3, double *tn3, double *tgn3, int mn2,
...
double
densu(double alt, double dlb, double tinf, double tlb, double xm, double alpha, double *tz, double zlb, double s2,
...
/*    Eq. A24a */
__inline_double
sg0(double ex, double *p, double *ap) {
    return (g0(ap[1],
               p)
            + (g0(ap[2],
                  p) * ex
               ...
               + g0(ap[4],
                 ...
                 * pow(ex,
                       3.0) +
               (g0(ap[5], p) * pow(ex, 4.0) + g0(ap[6], p) * pow(ex, 12.0)) * (1.0 - pow(ex, 8.0)) / (1.0 - ex)))
           / sumex(ex);
}
...
globe7(double *p, struct nrlmsise_input *input, struct nrlmsise_flags *flags) {
    ...
    /*  TIME INDEPENDENT */
    t[1] = (p[1] * plg[0][2] + p[2] * plg[0][4] + p[22] * plg[0][6]) + (p[14] * plg[0][2]) * dfa * flags->swc[1]
           + p[26] * plg[0][1];
    ...
    if (flags->sw[8]) {
        ...
        t[7] = f2
               * ((p[5] * plg[2][2] + p[41] * plg[2][4] + t81) * c2tloc + (p[8] * plg[2][2] + p[42] * plg[2][4] + t82)
                                                                          * s2tloc);
    }
    ...
    if (flags->sw[14]) {
        t[13] = f2
                * ((p[39] * plg[3][3] + (p[93] * plg[3][4] + p[46] * plg[3][6]) * cd14 * flags->swc[5]) * s3tloc
    ...
    if ((flags->sw[10]) && (input->g_long > -1000.0)) {
        ...
        if (flags->sw[12]) {
            t[11] = (1.0 + p[95] * plg[0][1]) * (1.0 + p[81] * dfa * flags->swc[1])
                    ...
                    * ((p[68] * plg[0][1] + p[69] * plg[0][3] + p[70] * plg[0][5]) * cos(sr * (input->sec - p[71])));
                    ...
                    t[12] = apt[0] * flags->swc[11] * (1. + p[132] * plg[0][1])
                            * ((p[52] * plg[1][2] + p[98] * plg[1][4] + p[67] * plg[1][6])
                            ...
                            + apt[0] * flags->swc[11] * flags->swc[5]
                              * (p[133] * plg[1][1] + p[134] * plg[1][3] + p[135] * plg[1][5]) * cd14
                              * cos(dgtr * (input->g_long - p[136]))
                            +
                            apt[0] * flags->swc[12] * (p[55] * plg[0][1] + p[56] * plg[0][3] + p[57] * plg[0][5]) * cos(
                                    sr * (input->sec - p[58]));
                }
            } else {
                t[12] = apdf * flags->swc[11] * (1.0 + p[120] * plg[0][1])
                        * ((p[60] * plg[1][2] + p[61] * plg[1][4] + p[62] * plg[1][6]) *
                        ...
                        + apdf * flags->swc[11] * flags->swc[5]
                          * (p[115] * plg[1][1] + p[116] * plg[1][3] + p[117] * plg[1][5]) * cd14
                          * cos(dgtr * (input->g_long - p[118]))
...
glob7s(double *p, struct nrlmsise_input *input, struct nrlmsise_flags *flags) {
    ...
    /* time independent */
    t[1] = p[1] * plg[0][2] + p[2] * plg[0][4] + p[22] * plg[0][6] + p[26] * plg[0][1] + p[14] * plg[0][3] + p[59]
                                                                                                             *
                                                                                                             plg[0][5];
    ...
    if (!((flags->sw[10] == 0) || (flags->sw[11] == 0) || (input->g_long <= -1000.0))) {
        t[10] =
                (1.0 + plg[0][1]
                       * (p[80] * flags->swc[5] * cos(dr * (input->doy - p[81])) + p[85] * flags->swc[6]
                                                                                   *
                                                                                   cos(2.0 * dr * (input->doy - p[86])))
                 + p[83] * flags->swc[3] * cos(dr * (input->doy - p[84])) + p[87] * flags->swc[4]
                                                                            * cos(2.0 * dr * (input->doy - p[88])))
                * ((p[64] * plg[1][2] + p[65] * plg[1][4] + p[66] * plg[1][6] + p[74] * plg[1][1] + p[75] * plg[1][3]
                    + p[76] * plg[1][5]) * cos(dgtr * input->g_long)
                   +
                   (p[90] * plg[1][2] + p[91] * plg[1][4] + p[92] * plg[1][6] + p[77] * plg[1][1] + p[78] * plg[1][3] +
                    p[79]
                    * plg[1][5]) * sin(dgtr * input->g_long));
    }
    tt = 0;
    for (i = 0; i < 14; i++)
...
gtd7(struct nrlmsise_input *input, struct nrlmsise_flags *flags, struct nrlmsise_output *output) {
    ...
    meso_tgn2[1] = pavgm[8] * pma[9][0]
                   * (1.0 + flags->sw[20] * flags->sw[22]
                            * glob7s(pma[9], input, flags)) * meso_tn2[3] * meso_tn2[3] /
    ...
    if (input->alt < zn3[0]) {
        ...
        meso_tgn3[1] = pma[7][0] * pavgm[7]
                       * (1.0 + flags->sw[22]
                                * glob7s(pma[7], input, flags)) * meso_tn3[4] * meso_tn3[4] /
    ...
    /**** Total mass density */
    output->d[5] = 1.66E-24
                   * (4.0 * output->d[0] + 16.0 * output->d[1] + 28.0 * output->d[2] + 32.0 * output->d[3] +
                      40.0 * output->d[4]
...
gts7(struct nrlmsise_input *input, struct nrlmsise_flags *flags, struct nrlmsise_output *output) {
    ...
    if (input->alt < 300.0) {
        ...
        meso_tgn1[1] = ptm[8] * pma[8][0]
                       * (1.0 + flags->sw[18] * flags->sw[20]
                                * glob7s(pma[8], input, flags)) * meso_tn1[4] * meso_tn1[4] /
    ...
    if ((flags->sw[15]) && (z < altl[0])) {
        ...
        zh04 = pdm[0][2];
        /*  Mixed density at Zlb */
        b04 = densu(zh04, db04, tinf, tlb, 4. - xmm, alpha[0] - 1., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
                    meso_tgn1);
        /*  Mixed density at Alt */
    ...
    if ((flags->sw[15]) && (z <= altl[1])) {
        ...
        zh16 = pdm[1][2];
        /*  Mixed density at Zlb */
        b16 = densu(zh16, db16, tinf, tlb, 16.0 - xmm, (alpha[1] - 1.0), &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
                    meso_tgn1);
        /*  Mixed density at Alt */
    ...
    if (flags->sw[15]) {
        if (z <= altl[3]) {
            ...
            zh32 = pdm[3][2];
            /*  Mixed density at Zlb */
            b32 = densu(zh32, db32, tinf, tlb, 32. - xmm, alpha[3] - 1., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
                        meso_tgn1);
            /*  Mixed density at Alt */
    ...
    if ((flags->sw[15]) && (z <= altl[4])) {
        ...
        zh40 = pdm[4][2];
        /*  Mixed density at Zlb */
        b40 = densu(zh40, db40, tinf, tlb, 40. - xmm, alpha[4] - 1., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
                    meso_tgn1);
        /*  Mixed density at Alt */
    ...
    if ((flags->sw[15]) && (z <= altl[6])) {
        ...
        zh01 = pdm[5][2];
        /*  Mixed density at Zlb */
        b01 = densu(zh01, db01, tinf, tlb, 1. - xmm, alpha[6] - 1., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
                    meso_tgn1);
        /*  Mixed density at Alt */
    ...
    if ((flags->sw[15]) && (z <= altl[7])) {
        ...
        zh14 = pdm[6][2];
        /*  Mixed density at Zlb */
        b14 = densu(zh14, db14, tinf, tlb, 14. - xmm, alpha[7] - 1., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
                    meso_tgn1);
        /*  Mixed density at Alt */
