...
G4NystromRK4::G4NystromRK4(G4Mag_EqRhs* magEqRhs, G4double distanceConstField) :
		...
		m_fEq(magEqRhs), m_magdistance(distanceConstField), m_cof(0.0), m_mom(0.0), m_imom(0.0), m_cachedMom(false)
{
...
void G4NystromRK4::Stepper(const G4double P[], const G4double dPdS[], G4double Step, G4double Po[], G4double Err[])
{
	G4double R[3] =
	{ P[0], P[1], P[2] };
	G4double A[3] =
	{ dPdS[0], dPdS[1], dPdS[2] };
	...
	//
	G4double K1[3] =
	{ m_imom * dPdS[3], m_imom * dPdS[4], m_imom * dPdS[5] };

	// Point2
	//
	G4double p[4] =
	{ R[0] + S5 * (A[0] + S4 * K1[0]), R[1] + S5 * (A[1] + S4 * K1[1]), R[2] + S5 * (A[2] + S4 * K1[2]), P[7] };
	getField(p);

	G4double A2[3] =
	{ A[0] + S5 * K1[0], A[1] + S5 * K1[1], A[2] + S5 * K1[2] };
	G4double K2[3] =
	{ (A2[1] * m_lastField[2] - A2[2] * m_lastField[1]) * m_cof, (A2[2] * m_lastField[0] - A2[0] * m_lastField[2]) * m_cof, (A2[0]
			* m_lastField[1] - A2[1] * m_lastField[0]) * m_cof };
	...
	//
	G4double A3[3] =
	{ A[0] + S5 * K2[0], A[1] + S5 * K2[1], A[2] + S5 * K2[2] };
	G4double K3[3] =
	{ (A3[1] * m_lastField[2] - A3[2] * m_lastField[1]) * m_cof, (A3[2] * m_lastField[0] - A3[0] * m_lastField[2]) * m_cof, (A3[0]
			* m_lastField[1] - A3[1] * m_lastField[0]) * m_cof };
	...
	G4double A4[3] =
	{ A[0] + S * K3[0], A[1] + S * K3[1], A[2] + S * K3[2] };
	G4double K4[3] =
	{ (A4[1] * m_lastField[2] - A4[2] * m_lastField[1]) * m_cof, (A4[2] * m_lastField[0] - A4[0] * m_lastField[2]) * m_cof, (A4[0]
			* m_lastField[1] - A4[1] * m_lastField[0]) * m_cof };
...
void G4NystromRK4::ComputeRightHandSide(const G4double P[], G4double dPdS[])
{
	G4double P4vec[4] =
	{ P[0], P[1], P[2], P[7] }; // Time is P[7]
	getField(P4vec);
