...
private:
    ...
    void
    given_altitude_particle_record(const G4Step *step);
    ...
    bool
    IDpart_not_recorded_yet(G4int ID);

    G4double
    Get_dist_rad(const G4double &lat, const G4double &lon, const G4double &alt_record);

    G4int nb_event = 0;

    G4double
    project_to_record_alt(const G4int type_number, const G4double &kinE, G4double &ecef_x, G4double &ecef_y,
                          G4double &ecef_z, const G4double &alt_in, const G4double &lat_in, const G4double &lon_in);
